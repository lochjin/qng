...
function get_txid_by_evmtxhash(){
  local etx_hash=$1
  local data='{"jsonrpc":"2.0","method":"getTxIDByMeerEVMTxHash","params":["'$etx_hash'"],"id":1}'
  get_result "$data"
}
...
function create_import_raw_tx() {
  local pkaddress=$1
  local amount=$2
  local data='{"jsonrpc":"2.0","method":"createImportRawTransaction","params":["'$pkaddress'",'$amount'],"id":1}'
  get_result "$data"
}
...
function create_export_raw_tx_v2() {
  local input=$1
  local data='{"jsonrpc":"2.0","method":"createExportRawTransactionV2","params":['$input'],"id":1}'
  get_result "$data"
}
...
function usage(){
  ...
  echo "  txv2 <id>"
  echo "  txbyhash <hash>"
  echo "  txidbyevmhash <evm tx hash>"
  echo "  createRawTx"
  echo "  createRawTxV2"
  echo "  createTokenRawTx"
  echo "  createExportRawTx <txid> <vout> <PKAdress> <amount>"
  echo "  createExportRawTxV2 <inputs> <outputs> <lockTime>"
...
elif [ "$1" == "txidbyevmhash" ]; then
  shift
  get_txid_by_evmtxhash $@
...
elif [ "$1" == "createExportRawTxV2" ]; then
  shift
  create_export_raw_tx_v2 $@
