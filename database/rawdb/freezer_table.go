...
func (t *freezerTable) repair() error {
	...
	for contentExp != contentSize {
		...
		if contentExp < contentSize {
			log.Warn("Truncating dangling head", "indexed", contentExp, "stored", contentSize)
		...
		if contentExp > contentSize {
			log.Warn("Truncating dangling indexes", "indexes", offsetsSize/indexEntrySize, "indexed", contentExp, "stored", contentSize)
	...
	if verbose {
		log.Info("Chain freezer table opened", "items", t.items.Load(), "deleted", t.itemOffset.Load(), "hidden", t.itemHidden.Load(), "tailId", t.tailId, "headId", t.headId, "size", t.headBytes)
	} else {
		log.Debug("Chain freezer table opened", "items", t.items.Load(), "size", common.StorageSize(t.headBytes))
	}
	return nil
}
...
func (t *freezerTable) repairIndex() error {
	...
	if t.metadata.version == freezerTableV1 {
		...
		log.Info("Recovering freezer flushOffset for legacy table", "offset", size)
	...
	case size > t.metadata.flushOffset:
		...
		log.Warn("Truncating freezer items after flushOffset", "size", extraSize)
	...
	default: // size < flushOffset
		...
		log.Warn("Rewinding freezer flushOffset", "old", t.metadata.flushOffset, "new", size)
		return t.metadata.setFlushOffset(size, true)
	}
}
...
func (t *freezerTable) truncateHead(items uint64) error {
	...
	if existing > items+1 {
		...
		log.Debug("Truncating freezer table", "items", existing, "limit", items)
	}
