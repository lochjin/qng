...
// Hash: d9c94e4de429f81e3a46508df5164bb1f2d1be5341780eeb2a9ae591586d08fb
// Version: 0.1.2
...
func (t *TransferData) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	// Offset (1) 'Stxos'
	dst = ssz.WriteOffset(dst, offset)
	for ii := 0; ii < len(t.Stxos); ii++ {
		offset += 4
		offset += t.Stxos[ii].SizeSSZ()
	}
	...
	// Field (1) 'Stxos'
	if size := len(t.Stxos); size > 24379 {
		err = ssz.ErrListTooBigFn("TransferData.Stxos", size, 24379)
		return
	}
	{
		offset = 4 * len(t.Stxos)
		for ii := 0; ii < len(t.Stxos); ii++ {
			dst = ssz.WriteOffset(dst, offset)
			offset += t.Stxos[ii].SizeSSZ()
		}
	...
	for ii := 0; ii < len(t.Stxos); ii++ {
		if dst, err = t.Stxos[ii].MarshalSSZTo(dst); err != nil {
			return
		}
	}
...
func (t *TransferData) UnmarshalSSZ(buf []byte) error {
	...
	{
		buf = tail[o1:o2]
		num, err := ssz.DecodeDynamicLength(buf, 24379)
		if err != nil {
			return err
		}
		t.Stxos = make([]*BytesData, num)
		err = ssz.UnmarshalDynamic(buf, num, func(indx int, buf []byte) (err error) {
			if t.Stxos[indx] == nil {
				t.Stxos[indx] = new(BytesData)
			}
			if err = t.Stxos[indx].UnmarshalSSZ(buf); err != nil {
				return err
			}
			return nil
		})
...
func (t *TransferData) HashTreeRootWith(hh ssz.HashWalker) (err error) {
	...
	{
		subIndx := hh.Index()
		num := uint64(len(t.Stxos))
		if num > 24379 {
			err = ssz.ErrIncorrectListSize
			return
		}
		for _, elem := range t.Stxos {
			if err = elem.HashTreeRootWith(hh); err != nil {
				return
			}
		}
		hh.MerkleizeWithMixin(subIndx, num, 24379)
	}
