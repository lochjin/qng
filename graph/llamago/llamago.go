...
func New(opts ...Option) (*LLM, error) {
	...
	client, err := llamagoclient.NewClient(o.ollamaServerURL, o.httpClient)
	if err != nil {
		return nil, err
	}
...
func (o *LLM) GenerateContent(ctx context.Context, messages []llms.MessageContent, options ...llms.CallOption) (*llms.ContentResponse, error) { // nolint: lll, cyclop, funlen
	...
	var fn llamagoclient.ChatResponseFunc
	streamedResponse := ""
	var resp llamagoclient.ChatResponse

	fn = func(response llamagoclient.ChatResponse) error {
		...
		if !req.Stream || response.Done {
			resp = response
			resp.Message = &llamagoclient.Message{
				Role:    "assistant",
...
func (o *LLM) CreateEmbedding(ctx context.Context, inputTexts []string) ([][]float32, error) {
	...
	for _, input := range inputTexts {
		req := &llamagoclient.EmbeddingRequest{
			Prompt: input,
			Model:  o.options.model,
		}
...
func makeLlamagoOptionsFromOptions(ollamaOptions llamagoclient.Options, opts llms.CallOptions) llamagoclient.Options {
