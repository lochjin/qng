...
func NewClient(ourl *url.URL, ohttp *http.Client) (*Client, error) {
	if ourl == nil {
		scheme, hostport, ok := strings.Cut(os.Getenv("LLAMAGO_HOST"), "://")
		if !ok {
			scheme, hostport = "http", os.Getenv("LLAMAGO_HOST")
		}

		host, port, err := net.SplitHostPort(hostport)
		if err != nil {
			host, port = "127.0.0.1", "8081"
			if ip := net.ParseIP(strings.Trim(os.Getenv("LLAMAGO_HOST"), "[]")); ip != nil {
				host = ip.String()
			}
		}
